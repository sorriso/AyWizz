...
**Last updated:** 2026-04-25 (`_observability` v2 livré : Docker events subscription. Le collector capture maintenant les containers démarrés APRÈS son `start()`. Avant : 7 services dans `/digest` (uniquement ceux up à _obs start). Après : 16 services (tous les c2..c9 + init containers). `_attach_to(container) -> bool` idempotent (set + lock) ; `_watch_events()` filter daemon-side `type=container, event=start` ; race init-scan-vs-events handled. 9 nouveaux tests unit collector. 672 unit/contract/coherence + 21 integration = 693 verts. Plus tôt aujourd'hui : workflow envelope synthesiser (Q-100-014 résolu) ; phases 1+2 structured logging + W3C trace propagation.
