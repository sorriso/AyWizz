...
**Last updated:** 2026-04-27 (**Plan v1 fonctionnel — Phase B livrée** : upload + parsers réels. Endpoint `POST /api/v1/memory/projects/{p}/sources/upload` multipart avec persist blob MinIO + parsing. 5 parsers actifs (text/plain, text/markdown, text/html via BS4, application/pdf via pypdf, DOCX via python-docx) ; image MIMEs retirés du contrat (réservés v1.5+ OCR). 7 tests dirigés (1 par parser + corrupt-PDF 422 + unsupported-mime 415). Refactor service : pipeline post-parse extrait dans `_index_parsed_source` partagé entre flow JSON et flow multipart. Catalog auth-matrix +1 endpoint, doc 065 régénérée (71). Deps ajoutées : pypdf, beautifulsoup4, python-docx (lxml transitif). CI 1133 verts en 130s. Plus tôt aujourd'hui : Phase A (tenant+project), Phase C (Ollama embedder), auth-matrix Phases 1+2, Q-100-015. Reste plan v1 : Phase D (chat-with-RAG) → E (conversation→memory) → F (KG extraction).
...
1. **Phase D — Chat-with-RAG dans C3** (~2 sessions). Étendre `POST /api/v1/conversations/{id}/messages` : retrieve C7 → augment prompt → C8 LLM → stream SSE → persist. Consomme les chunks indexés par Phase B + l'embedder Ollama Phase C.
2. **Phase E — Conversation → memory loop** (~1 session). Ingestion automatique des messages assistant dans C7 avec `source_type=conversation` ; retrieve scope `uploads`/`conversations`/`all`.
3. **Phase F — Knowledge graph extraction F.1** (~1-2 sessions). LLM-based entity+relation extraction → `memory_kg_entities` + `memory_kg_relations` ; hybrid retrieval F.2 différé v1.5.
4. **Devcontainer rebuild** pour `testcontainers/ryuk:0.5.x` — durable au leak Docker (sinon wrapper post-mortem nécessaire).
5. **R-100-060 — production K8s manifests** : déféré jusqu'à v1 fonctionnel complet (post Phase F).
...
- `.claude/sessions/2026-04-27-phase-b-upload-parsers.md` — **Plan v1 Phase B**. Endpoint multipart `POST /api/v1/memory/projects/{p}/sources/upload`. 5 parsers actifs (txt, MD, HTML via BS4, PDF via pypdf, DOCX via python-docx). Blob MinIO `sources/{tenant}/{project}/{source_id}{.ext}`. Refactor service : `_index_parsed_source` shared entre flow JSON et flow multipart. 7 tests dirigés + auto-paramétrés sur catalog. Image MIMEs retirés du contrat (réservés v1.5+ OCR). Deps : pypdf, beautifulsoup4, python-docx. CI 1133 verts.
- `.claude/sessions/2026-04-27-phase-c-ollama-embedder.md` — **Plan v1 Phase C**. Switch `.env.example` C7_EMBEDDING_* vers `ollama`/`all-minilm`/`384` (production default). `.env.test` était déjà aligné. Ouvre la voie à un retrieval sémantique réel pour la RAG. Section commentaire ajoutée pour expliciter que `deterministic-hash` est test-only. Tests slow `test_real_embedder.py` 3 verts (top-1 = src-cat sur query sémantique cat-vs-rocket). Décision tracée §3 de cette SESSION-STATE (CLAUDE.md §4.6).
- `.claude/sessions/2026-04-27-phase-a-tenant-project-lifecycle.md` — **Plan v1 Phase A**. 8 nouveaux endpoints C2 (admin_router pour tenant_manager, projects_router pour admin/owner). Nouvelle collection `c2_projects`. 11 méthodes repo + 8 service + 6 tests dirigés round-trip (tenant→projet→grant→login propage `project_scopes`). Catalog auth-matrix 62→70. CI 1121 verts. tenant_manager content-blindness enforcée stricte.
- `.claude/sessions/2026-04-27-auth-matrix-phase2.md` — Phase 2 auth-matrix : 99 nouveaux tests (role_matrix 63 + isolation 27 + backend_state 4 + auth_modes 5). Helpers `_backend.py` Arango/MinIO. **Hang root cause** : pytest-asyncio session fixture sans `loop_scope="session"` → loop éphémère fermé après ~20 tests cumulatifs. Fix appliqué partout. **Cleanup wrapper** `docker_test_cleanup.sh` allowlisté (settings.json v8→v9, 5 forms) — 8 conteneurs orphelins (Ryuk absent du devcontainer) supprimés. SSO mode test = stub 501, full-flow JWKS-mocked différé jusqu'au déploiement `oauth2-proxy`. CI 1084 verts.
- `.claude/sessions/2026-04-27-auth-matrix-framework.md` — Phase 1 auth-matrix. E-100-002 v1→v2 (5-rôles : `tenant_manager` super-root content-blind + `admin`/`tenant_admin` tenant-scoped + 3 project roles). `RBACGlobalRole.TENANT_MANAGER` ajouté. Catalog-driven framework `tests/e2e/auth_matrix/` : 62 endpoints catalogués, stack 7-composants (1 Arango + 1 MinIO), 62 tests anonymous_access auto-paramétrés. Coherence script. Doc auto-générée `065-TEST-MATRIX.md` + generator script. CLAUDE.md v19→v20 §13. 100-SPEC v12→v13.
- `.claude/sessions/2026-04-27-q-100-015-loki-es-adapters.md` — Q-100-015 résolu. R-100-124 (Production Workflow Synthesis Service) : `SpanSource` Protocol + Buffer/Loki/ES adapters + `make_workflow_router` montable. `_observability/main.py` v2 délègue au router partagé. 33 unit + 6 integration tests (testcontainers Loki + ES single-node). LogQL pipeline `| json | event="span_summary"` (whitespace-tolerant). 100-SPEC v11→v12, Q-100-017/018 ouverts. CI 919 verts, coverage 90.23%.
- `.claude/sessions/2026-04-27-claude-md-v19-and-test-only-cleanup.md` — CLAUDE.md v18→v19 §12 (`run_tests.sh ci` discipline). 2 markers ajoutés (R-100-080/081 → c7_memory/router.py + workflow JSON). Audit script étend `*.json`. 8 test-only restants documentés comme légitimes (meta-rules, test-as-impl, WIP stubs). `run_tests.sh ci` = All stages OK.
...
- _Earlier 2026-04-22..24 entries_ : C1 gateway Traefik, C2 auth, C3 conversation, C5 requirements v1+v1.5, C6 validation, C7 memory, C8 LLM, C9 MCP, C12 n8n. Governance: matcher-friendly shell §5.7, test debug §10, coverage §11, sed ban §5.2/§4.6, e2e wrapper §5.3, e2e stack infra, script path forms, env discipline. See `sessions/2026-04-22-*.md`, `sessions/2026-04-23-*.md`, `sessions/2026-04-24-*.md`.
